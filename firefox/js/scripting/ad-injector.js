...
(function () {
    ...
    function createAdContainer(ad) {
        ...
        // Use title or index as unique identifier
        const adId = ad.id || ad.title || `ad-${Date.now()}`;
        wrapper.setAttribute('data-custom-ad-id', adId);
    ...
    function injectAd(slot, ad) {
        try {
            ...
            // Skip if ad already injected (use title or index as unique identifier)
            const adId = ad.id || ad.title || `ad-${injectedAdIds.size}`;
            if (injectedAdIds.has(adId)) {
                return false;
            }
            ...
            requestAnimationFrame(() => {
                ...
                const adId = ad.id || ad.title || `ad-${injectedAdIds.size}`;
                injectedAdIds.add(adId);
                injectedContainers.push(adContainer);
                console.log(`[AdInjector] Injected ad: ${ad.title || adId}`);
            });
