...
(function () {
    ...
    async function fetchAds(domain) {
        ...
        try {
            const url = `${CONFIG.API_BASE_URL}/api/ads?domain=${encodeURIComponent(domain)}`;
            console.log(`[AdManager] Fetching ads from ${url}`);
    ...
    async function logAdEvent(domain) {
        try {
            ...
            const url = `${CONFIG.API_BASE_URL}/api/extension/log`;
            ...
            if (!response.ok) {
                throw new Error(`Log API returned ${response.status}: ${response.statusText}`);
            }

            console.log(`[AdManager] Logged ad event for ${domain}`);
        } catch (error) {
            console.error(`[AdManager] Failed to log ad event for ${domain}:`, error);
            // Don't throw - logging failure shouldn't break ad injection
        }
    }
